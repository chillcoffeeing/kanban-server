...
const cardPlan = [
  ...
  },
  {
    ...
    description:
      "Reemplazar lucide-react por @phosphor-icons/react y normalizar tamanos.",
    ownerIndex: 1,
  },
  {
    stageIndex: 0,
  ...
  {
    stageIndex: 0,
    title: "Historial de actividad del tablero",
    description:
      "Mostrar feed en la sidebar con eventos recientes (create/move/comment).",
    ownerIndex: 0,
  },
  ...
  {
    stageIndex: 2,
    title: "Filtros por miembro y label",
    description:
      "Query params en URL; estado sincronizado con el store. Pendiente de revision UX.",
    ownerIndex: 0,
  },
  {
    stageIndex: 2,
    title: "Refactor del sidebar colapsable",
    description:
      "Extraer <Sidebar /> y persistir estado collapsed por usuario. Listo para review de Alice.",
    ownerIndex: 2,
  },
  ...
  {
    stageIndex: 3,
    title: "Setup inicial Vite + React 19",
  ...
  {
    stageIndex: 3,
    title: "Tema oscuro con toggle",
    description:
      "Toggle persistido en preferencias del usuario; tokens para ambos modos.",
    ownerIndex: 2,
  },
];
...
async function main() {
  ...
  if (shouldReset) {
    ...
    for (const existingBoard of existingBoards ?? []) {
      if (
        ...
        existingBoard.ownerId === aliceAccount.user.id
      ) {
        ...
        console.log(
          `[reset] board previo borrado (${existingBoard.id.slice(0, 8)})`,
        );
      }
    }
  }
  ...
  for (const invitation of invitations) {
    const { data: createdInvitation } = await httpRequest(
      "POST",
      `/boards/${createdBoard.id}/invitations`,
      {
        token: aliceAccount.token,
    ...
    await httpRequest(
      "POST",
      `/invitations/${createdInvitation.token}/accept`,
      {
    ...
    console.log(
      `${invitation.account.name} acepto invitacion (${invitation.role})`,
    );
  }
  ...
  for (const plannedCard of cardPlan) {
    ...
    const cardResponse = await httpRequest(
      "POST",
      `/stages/${stageIds[plannedCard.stageIndex]}/cards`,
      {
        token: author.token,
        body: {
    ...
    if (cardResponse.status !== 201 && cardResponse.status !== 200) {
      console.error(
        `[FAIL] ${plannedCard.title} respondio ${cardResponse.status}`,
        cardResponse.data,
      );
      continue;
    }
    console.log(
      `  [${stageNames[plannedCard.stageIndex].padEnd(11)}] ${plannedCard.title}  by ${author.name.split(" ")[0]}`,
    );
